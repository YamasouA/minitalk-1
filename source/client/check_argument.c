...
{
	if (argc != 3)
		exit_client(USAGE_MSG_HEADER USAGE_MSG_DESCRIPTION, false);
	if (!check_pid_server_is_digit(argv[1]))
		exit_client(ERROR_MSG_HEADER ERROR_MSG_PID_TYPE, false);
	if (!check_pid_server_is_in_range(ft_atoi(argv[1])))
		exit_client(ERROR_MSG_HEADER ERROR_MSG_PID_RANGE, false);
	if (!check_message_is_string(argv[2]))
		exit_client(ERROR_MSG_HEADER ERROR_MSG_MSG_INVALID, false);
}
